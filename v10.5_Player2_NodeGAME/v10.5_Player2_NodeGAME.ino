 ...
 */
   ...
   String projectName = "NodeGAME+ v10.5";
  String software_version = "v10.5";
  String player_ID = "P2";
...
/* IMPORTANT -------------> HTTP Web Server CREDENTIAL CORE
 *  
 * This is the variable that will be used for HTTP Web Server Connectivity.
 ...
 * NodeMCU ESP8266 IP Address for testarduino --> 192.168.1.4
 * 
 */

      //Wifi HP
    const char* ssid = "sidanglulus";    // Enter SSID here
...
//      // Wifi Kost
//    const char* ssid = "PERMATA B18";    // Enter SSID here
//    const char* password = "indonesia";    // Enter Password here
...
void play_PingPong_Multi()  { 
  //////////////////////PINGPONG POST HTTP REQUEST///////////////////////

  StaticJsonBuffer<300> JSONbuffer;
  JsonObject& JSONencoder = JSONbuffer.createObject();

  JSONencoder["dataType"] = "Load_Position";
  JsonArray& values = JSONencoder.createNestedArray("values");

  values.add(loading_pingpong);
  values.add(playerTwo_Position);

  char JSONmessageBuffer[100];
  JSONencoder.printTo(JSONmessageBuffer, sizeof(JSONmessageBuffer));
  Serial.println("Sending message to HTTP Web Server..");
  Serial.println(JSONmessageBuffer);
      
   HTTPClient http;    //Declare object of class HTTPClient
 
//   http.begin("http://192.168.1.2:1880/pingpong/p2/post/jsondata");      //Specify request destination
   http.begin("http://192.168.43.170:1880/pingpong/p2/post/jsondata");      //Specify request destination
   http.addHeader("Content-Type", "application/json");  //Specify content-type header
 
   int p2_sendJSON = http.POST(JSONmessageBuffer);                                   //Send the request
   String p2_payloadJSON = http.getString();                                        //Get the response payload

    http.end();
  //////////////////////PINGPONG GET HTTP REQUEST///////////////////////

      http.begin("http://192.168.43.170:1880/pingpong/p1/get/jsondata");
//      http.begin("http://192.168.1.2:1880/pingpong/p1/get/jsondata");
   int p1_JSONGet = http.GET();


  if(p1_JSONGet == 200) {
   String p1_JSONMessage = http.getString();
    Serial.print("Initial string value: "); Serial.println(p1_JSONMessage);

    StaticJsonBuffer<300> JSONBuffer;   //Memory pool
    JsonObject& parsed = JSONBuffer.parseObject(p1_JSONMessage); //Parse message
...
//    int arraySize = parsed["values"].size();   //get size of JSON Array
//    Serial.print("\nSize of values array: ");
//    Serial.println(arraySize);

    p1_ready = parsed["values"][0];
    ball_coorX = parsed["values"][1];
    ball_coorY = parsed["values"][2];
    playerOne_Position = parsed["values"][3];
    playerOne_Score = parsed["values"][4];
    playerTwo_Score = parsed["values"][5];
  }
    http.end();
//////////////////////////////////////////////////////////////////////////////
...
//    http.begin("http://192.168.1.2:1880/tictactoe/player_o/post/signal");
...
//  http.begin("http://192.168.1.2:1880/tictactoe/player_x/get/signal");
...
//  http.begin("http://192.168.1.2:1880/tictactoe/player_x/get/row");
...
//  http.begin("http://192.168.1.2:1880/tictactoe/player_x/get/column");
...
//  http.begin("http://192.168.1.2:1880/tictactoe/player_x/get/turn");
...
//            http.begin("http://192.168.1.2:1880/tictactoe/player_o/post/row");
...
//            http.begin("http://192.168.1.2:1880/tictactoe/player_o/post/column");
...
//            http.begin("http://192.168.1.2:1880/tictactoe/player_o/post/turn");
